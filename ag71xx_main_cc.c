...
extern int board_use_ag71xx_cc;
...
void ag71xx_link_adjust_cc(struct ag71xx *ag)
{
...
static int ag71xx_cc_probe(struct platform_device *pdev)
{
	struct net_device *dev;
	struct resource *res;
	struct ag71xx *ag;
...
static int ag71xx_cc_remove(struct platform_device *pdev)
{
...
static struct platform_driver ag71xx_cc_driver = {
	.probe		= ag71xx_cc_probe,
	.remove		= ag71xx_cc_remove,
	.driver = {
		.name	= AG71XX_CC_DRV_NAME,
	}
};

static int __init ag71xx_cc_module_init(void)
{
	int ret;
	...
	ret = platform_driver_register(&ag71xx_cc_driver);
	if (ret)
		goto err_mdio_exit;
...
static void __exit ag71xx_cc_module_exit(void)
{
	platform_driver_unregister(&ag71xx_cc_driver);
	ag71xx_mdio_driver_exit();
	ag71xx_debugfs_root_exit();
}

module_init(ag71xx_cc_module_init);
module_exit(ag71xx_cc_module_exit);

MODULE_VERSION(AG71XX_DRV_VERSION);
MODULE_AUTHOR("Gabor Juhos <juhosg@openwrt.org>");
MODULE_AUTHOR("Imre Kaloz <kaloz@openwrt.org>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" AG71XX_CC_MODULE_NAME);
